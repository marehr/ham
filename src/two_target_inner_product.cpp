...
int main([[gnu::unused]]int argc, [[gnu::unused]]char* argv[])
{
	...
	std::string host_name = host.str();

	std::cout << host_name << "Using target node " << target1 << " with hostname " << target1_description.name() << std::endl;
	std::cout << host_name << "Using target node " << target2 << " with hostname " << target2_description.name() << std::endl;
	...
	double c = c_future1.get() + c_future2.get();
